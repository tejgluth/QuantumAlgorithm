...
- **Baselines**: `qiskit_sabre_best`, `sabre_layout_swap`, `qiskit_sabre_trials8` (fair multi-trial SABRE with swap/depth selection), `teacher_sabre_like`, IL and RL checkpoints, residual model; WS uses trials=8, `snapshot_mode=avg`.
...
- **Paired deltas (Fig. 2, `significance.csv`)**: mean Δ = +0.0311, 95% CI [+0.0283, +0.0340]; 84.6% of paired draws favor WS; Wilcoxon p < 1e‑10; Cliff’s δ = 0.72 (large; per-graph δ: 0.82/0.60/0.77 for grid/heavy_hex/ring).
- **Significance/effect size table**: `artifacts/tables/significance_effect.csv` reports per-graph Δ with 95% CI, exact p-values (appendix only), positive fraction, and Cliff’s δ labels.
- **Trial-matched SABRE (`qiskit_sabre_trials8`)**: swap/depth-based selection gains over the single-trial baseline, but WS still leads overall (+0.008 mean Δ overall_log_success; +0.029 on grid_3x3, +0.016 on ring_8, −0.016 on heavy_hex_15). Swap ratios vs this baseline are 1.17/1.23/1.48 (grid/ring/heavy), with duration ratios 1.01–1.07.
...
## Mechanism
Ablations that zero out α/β weighting leave the mean paired improvement unchanged, indicating that **objective-aware selection among multiple SABRE trials, scored by a drift-aware success proxy, is the main driver of gains**. This is a compiler decision pattern: generate several valid routes under heterogeneous hardware assumptions, then select the one that optimises the downstream objective (expected success) rather than a structural surrogate (swap/depth). Local edge re-weighting provides a small bias but is secondary to the multi-trial + proxy selection loop.

## Limitations and Scope
Synthetic-but-structured hardware draws let us stress drift, directionality, and crosstalk in a reproducible way for expected-yield analysis, but they do not substitute for live calibration data. To replicate on public calibration snapshots, one would need (1) ingestion of calibration JSON for multiple dates, (2) mapping of gate errors, durations, T1/T2, and readout into `HardwareModel` fields, and (3) repetition of the evaluation across those snapshots to observe day-to-day drift. Claims here are restricted to the synthetic-yet-realistic profile described above; no direct hardware success rates are reported.
